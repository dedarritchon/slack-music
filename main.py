...
from models.weekly_polls import WeeklyPoll, SongInfo, PollResults
import re
...
async def update_home_tab_view(client, app_user: User, weekly_poll: WeeklyPoll, logger):
    ...
    if poll_status == "submissions_open":
        ...
        if not await user_has_voted(app_user.id, weekly_poll.poll_id):
            view_blocks.append({
                "type": "section",
                "text": {
                    "type": "mrkdwn",
...
async def user_has_voted(user_id, poll_id):
...
async def get_voting_options(weekly_poll: WeeklyPoll) -> List[SongInfo]:
    ...
    return weekly_poll.songs.values()  # Assuming songs is a dictionary of SongInfo objects
